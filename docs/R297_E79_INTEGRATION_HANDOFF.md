...
- ⑤: Windows-native Producer still verifies recovered output at current time;
  wire a trusted, original-event receipt result via the authorized Windows relay
  before enabling recovery beyond five minutes. Shared Windows hardlink recovery
  remains explicitly stopped until the real ACL boundary implementation is
  delivered and tested. No Linux socket assumption on Windows.
