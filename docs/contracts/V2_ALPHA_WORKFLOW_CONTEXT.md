...
- 失败状态必须保留失败原因。
- 恢复必须生成新的 recovery Trace 与 Root Span，但仍复用同一业务 `workflow_id`、`task_id` 和 `recovered_from_run_id`。
- 恢复不得重复生成正式知识资产。
- 恢复不得重复调用同一技能版本，除非显式标记为“重试”。
- 恢复不得重新创建 Task、Research Execution、Knowledge Asset、Skill Invocation 或审计根节点。
- 每个 Child Span 必须包含 `trace_id`、`span_id`、`parent_span_id`、`module_name`、`started_at`、`finished_at` 与 `status`。
