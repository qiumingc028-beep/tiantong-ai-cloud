...
if ($env:GITHUB_EVENT_PATH -and (Test-Path -LiteralPath $env:GITHUB_EVENT_PATH)) {
  ...
  if ($event.PSObject.Properties.Name -contains 'pull_request') {
    $expectedSource = [string]$event.pull_request.head.sha
  }
}
Require ($env:RELEASE_SOURCE_SHA -eq $head) 'EVIDENCE_COMMIT_MUST_EQUAL_RELEASE_SOURCE_SHA'
Require ($expectedSource -eq $head) 'CHECKOUT_MUST_EQUAL_WORKFLOW_SOURCE_HEAD'
