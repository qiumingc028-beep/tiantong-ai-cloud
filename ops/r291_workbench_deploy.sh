...
if [[ $# -ne 2 ]]; then
  echo "usage: r291_workbench_deploy.sh DEPLOYMENT_DIR EXPECTED_COMMIT" >&2
  exit 64
fi

DEPLOYMENT_DIR=$(realpath "$1")
EXPECTED_COMMIT=$2
ENV_FILE=${R291_ENV_FILE:?set R291_ENV_FILE to the root-only production environment file}
PROJECT_NAME=tiantong_v2_s12_internal
COMPOSE_FILE=$DEPLOYMENT_DIR/docker-compose.prod.yml
BACKUP_ROOT=/opt/tiantong-v2-s12/backups/r291
...
rollback() {
  ...
  if [[ $ROLLBACK_ACTIVE -eq 1 \
    ...
    if [[ $rollback_healthy -eq 1 ]]; then
      echo "R291_AUTOMATIC_IMAGE_CONFIG_ROLLBACK_COMPLETED" >&2
    else
      echo "R291_AUTOMATIC_IMAGE_CONFIG_ROLLBACK_HEALTHCHECK_FAILED" >&2
    fi
  fi
  exit "$exit_code"
}
...
bash "$DEPLOYMENT_DIR/ops/r291_workbench_preflight.sh" "$DEPLOYMENT_DIR" "$EXPECTED_COMMIT"
SOURCE_DATE_EPOCH=$(git -C "$DEPLOYMENT_DIR" show -s --format=%ct "$EXPECTED_COMMIT")
[[ $SOURCE_DATE_EPOCH =~ ^[0-9]+$ ]] || { echo "R291_SOURCE_DATE_EPOCH_INVALID" >&2; false; }
...
running_config_files=$(docker inspect \
  ...
  "$running_backend_id")
[[ -n $running_config_files ]] || { echo "R291_RUNNING_COMPOSE_LABEL_MISSING" >&2; false; }
...
for running_compose_path in "${running_compose_paths[@]}"; do
  ...
  [[ -f $running_compose_path ]] || { echo "R291_RUNNING_COMPOSE_SOURCE_MISSING" >&2; false; }
...
for service in backend worker nginx; do
  ...
  [[ -n $built_image ]] || { echo "R291_BUILT_IMAGE_MISSING" >&2; false; }
  built_arch=$(docker image inspect --format '{{.Architecture}}' "$built_image")
  [[ $built_arch == amd64 ]] || { echo "R291_NON_NATIVE_IMAGE_BLOCKED" >&2; false; }
  built_commit=$(docker image inspect --format '{{ index .Config.Labels "org.opencontainers.image.revision" }}' "$built_image")
  [[ $built_commit == "$EXPECTED_COMMIT" ]] || { echo "R291_IMAGE_COMMIT_LABEL_MISMATCH" >&2; false; }
done
...
postgres_network=$(docker inspect \
  ...
  | head -n 1)
[[ -n $migration_image ]] || { echo "R291_MIGRATION_IMAGE_MISSING" >&2; false; }
[[ -n $postgres_network ]] || { echo "R291_POSTGRES_NETWORK_MISSING" >&2; false; }
...
if any(not os.environ.get(name) for name in required):
    raise SystemExit("R291_MIGRATION_ADMIN_ENV_MISSING")
...
[[ $healthy -eq 1 ]] || { echo "R291_PUBLIC_HEALTHCHECK_FAILED" >&2; false; }
...
if not password:
    raise SystemExit("R291_BOSS_LOGIN_SECRET_MISSING")
...
login_request = urllib.request.Request(
    ...
    headers={"Content-Type": "application/json", "User-Agent": "Tiantong-R291-Deploy-Probe"},
    method="POST",
)
with opener.open(login_request, timeout=15) as response:
    if response.status != 200:
        raise SystemExit("R291_BOSS_LOGIN_FAILED")
with opener.open("https://internal.tiantongai.com/api/me", timeout=15) as response:
    user = json.load(response)
if user.get("role_code") != "owner":
    raise SystemExit("R291_BOSS_LOGIN_IDENTITY_MISMATCH")
PY
