...
test('all 320 protected-page handlers are migrated without inline event code',()=>{
  const root=new URL('../frontend/',import.meta.url);
  const visit=dir=>readdirSync(dir,{withFileTypes:true}).flatMap(entry=>entry.isDirectory()?visit(new URL(`${entry.name}/`,dir)):[new URL(entry.name,dir)]);
  ...
  assert.equal(migrated,320);
  assert.equal(literalActions,255);
  assert.equal(dynamicActions,65);
  assert.equal(migratedHandlerIds.size,320);
