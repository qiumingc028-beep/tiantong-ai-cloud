...
def test_worker_process_next_task_does_not_exit_on_redis_timeout(monkeypatch):
    ...
    monkeypatch.setattr(worker, "claim_task", raise_timeout)
...
def test_worker_process_next_task_does_not_exit_on_redis_connection_error(monkeypatch):
    ...
    monkeypatch.setattr(worker, "claim_task", raise_connection_error)
    monkeypatch.setattr(worker.time, "sleep", lambda seconds: None)
...
def test_worker_does_not_ack_after_database_lease_was_recovered(monkeypatch):
    ...
    monkeypatch.setattr(worker, "_claim_jd_workbench_task", lambda *_args: True)
    monkeypatch.setattr(worker, "_TaskHeartbeat", NoopHeartbeat)
    monkeypatch.setattr(worker, "handle_task", lambda _task: None)
    monkeypatch.setattr(worker, "_finish_jd_workbench_task", lambda *_args, **_kwargs: False)
    monkeypatch.setattr(worker, "ack_task", lambda *_args: acked.append(True))
