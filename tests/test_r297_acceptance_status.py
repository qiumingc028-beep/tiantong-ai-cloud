...
from backend.models import JdSyncLog, Store, EmployeeLog
...
def test_observation_counts_only_scoped_successful_cloud_windows(client, owner_headers, test_db):
    ...
    assert set(result) == {'release_sha', 'namespace', 'tenant_id', 'company_id', 'store_id', 'platform',
                           'completed_cycle_count', 'observed_at'}
    with test_db() as db:
