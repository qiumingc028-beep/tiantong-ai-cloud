...
class SecurityIncident(Base):
    ...
    incident_code: Mapped[str] = mapped_column(String(80), nullable=False, unique=True, index=True)
    ...
    description: Mapped[str | None] = mapped_column(Text)
    detected_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, server_default=func.now(), index=True)
    detected_by: Mapped[str | None] = mapped_column(String(80), index=True)
    evidence_references_json: Mapped[str | None] = mapped_column(Text)
    risk_score: Mapped[int] = mapped_column(Integer, nullable=False, default=0, index=True)
    automatic_action: Mapped[str | None] = mapped_column(String(80), index=True)
    assigned_to: Mapped[str | None] = mapped_column(String(80), index=True)
    acknowledged_by: Mapped[str | None] = mapped_column(String(80), index=True)
    acknowledged_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), index=True)
    resolved_by: Mapped[str | None] = mapped_column(String(80), index=True)
...
class AlertRule(Base):
    ...
    rule_code: Mapped[str] = mapped_column(String(80), nullable=False, unique=True, index=True)
    metric_name: Mapped[str] = mapped_column(String(120), nullable=False, index=True)
...
class ExecutionReplayIndex(Base):
    ...
    workflow_id: Mapped[str] = mapped_column(ForeignKey("computer_workflows.workflow_id", ondelete="CASCADE"), nullable=False, unique=True, index=True)
    task_id: Mapped[int | None] = mapped_column(ForeignKey("task_center_tasks.id", ondelete="SET NULL"), index=True)
    execution_id: Mapped[str | None] = mapped_column(String(120), index=True)
    session_id: Mapped[str | None] = mapped_column(ForeignKey("computer_sessions.session_id", ondelete="SET NULL"), index=True)
...
class EmployeeExecutionMetric(Base):
    ...
    metric_id: Mapped[str] = mapped_column(String(36), primary_key=True)
    employee_code: Mapped[str] = mapped_column(String(80), nullable=False, unique=True, index=True)
    employee_name: Mapped[str | None] = mapped_column(String(120), index=True)
    total_tasks: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    success_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    success_rate: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    avg_quality_score: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    ...
    verification_fail_rate: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    takeover_rate: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    security_incident_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    budget_exceeded_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    canceled_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
...
class DeviceHealthScore(Base):
    ...
    device_id: Mapped[str] = mapped_column(ForeignKey("devices.device_id", ondelete="CASCADE"), nullable=False, unique=True, index=True)
    score: Mapped[int] = mapped_column(Integer, nullable=False, index=True)
    grade: Mapped[str] = mapped_column(String(20), nullable=False, index=True)
    dimension_scores_json: Mapped[str] = mapped_column(Text, nullable=False)
